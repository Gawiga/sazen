...
describe('API Endpoints - Auth', () => {
  ...
  describe('Token validation', () => {
    it('should validate non-expired token', () => {
      ...
      const validToken = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpZCI6InVzZXIxIiwiZXhwIjo0MDAwMDAwMDAwfQ==.sig';
    ...
    it('should reject expired token', () => {
      ...
      const expiredToken = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpZCI6InVzZXIxIiwiZXhwIjoxMDAwMDAwMDAwfQ==.sig';
...
describe('API Endpoints - CRUD Operations', () => {
  describe('Pagination', () => {
    it('should parse pagination query parameters', () => {
      const url = new URL('https://api.example.com/api/reports?collection=faturamento_mensal&page=2&perPage=25');
    ...
    it('should use default pagination values', () => {
      const url = new URL('https://api.example.com/api/reports?collection=values');
    ...
    it('should clamp perPage to range [1-100]', () => {
      const clampPerPage = (perPage: number) => Math.max(1, Math.min(100, perPage));
    ...
    it('should calculate skip offset correctly', () => {
      const calculateSkip = (page: number, perPage: number) => (page - 1) * perPage;
  ...
  describe('Response format', () => {
    ...
    it('should calculate totalPages correctly', () => {
      const calculateTotalPages = (totalItems: number, perPage: number) => Math.ceil(totalItems / perPage);
...
describe('API Endpoints - CRUD Validation', () => {
  describe('Pacientes endpoint', () => {
    it('should validate required fields for create', () => {
      const validatePaciente = (data: any) => {
        ...
        if (typeof data.valor_sessao !== 'number') errors.valor_sessao = 'Valor is required';
      ...
      const result = validatePaciente({ nome: 'John', email: 'j@ex.com', valor_sessao: 50 });
      expect(result.valid).toBe(true);
  ...
  describe('Sessões endpoint', () => {
    it('should validate required fields for session', () => {
      ...
      const valid = validateSessao({ id_paciente: 'p1', data: '2024-01-01', valor: 50 });
